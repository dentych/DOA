...
public:
	...
	void insert(const T& key)
	{
		...
		while (map[index].state == MapEntry<T>::OCCUPIED) index = (index + 1) % size;	// Linear (anal) probing
...
private:
	...
	{
		// YOUR IMPLEMENTATION GOES HERE!!! OK OK. CALM DOWN MAN
		size_t firstIndex;
		index = hash(key);
		...
		do {
			if (map[index].key == key && map[index].state == MapEntry<T>::OCCUPIED) {
				return true;
			}
			else {
				index = (index + 1) % size; // Linear (anal) probing
			}
		} while (map[index].state != MapEntry<T>::PRISTINE && index != firstIndex);
		return false;
	}
...
template <>
size_t OAHashMap<std::string>::hash(const std::string & key) {
	return ((key[0] / 10) + 2) % size;
}

template class OAHashMap<std::string>;

template <typename T>
size_t OAHashMap<T>::hash(const T & key) {
	return ((key / 10) + 2) % size;
}